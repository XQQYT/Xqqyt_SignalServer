...
// 异步读取客户端消息
void Server::async_read_message(std::shared_ptr<websocket::stream<tcp::socket>> client_ws, int client_socket) {
    beast::flat_buffer buffer;
    
    client_ws->async_read(buffer, [this, client_ws, client_socket, buffer](boost::system::error_code ec, std::size_t bytes_transferred) {
        if (ec) {
        ...
        std::cout << "Received client message: " << received_message << std::endl;
        ...
        // 继续异步读取
        async_read_message(client_ws, client_socket);
    });
}
    ...
    while (true) {
        ...
        for (int i = 0; i < ready_num; i++) {
            ...
            if (cur_fd == listen_socket) {
                ...
                auto client_ws = sd_clients[cur_fd];
                async_read_message(client_ws, cur_fd);
            }
        }
    }
}
